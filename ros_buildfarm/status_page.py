...
def build_blocked_source_entries_page(
    ...
    rosdistro_name = _rosdistro_or_latest(index, rosdistro_name)
    ...
    # has a source entry? Lets call that 'released' with a 'version' to reuse
    # logic for the blocked releases page
...
def _rosdistro_or_latest(index, rosdistro_name=None):
    """Return rosdistro_name if specified, else return lastest distro."""
    if rosdistro_name is None:
        valid_rosdistro_names = list(index.distributions.keys())
        valid_rosdistro_names.sort()
        rosdistro_name = valid_rosdistro_names[-1]
    return rosdistro_name
...
def _package_dependencies(walker, package):
    try:
        yield from walker.get_recursive_depends(
...
def _released_packages(distro, repo_names=None):
    if repo_names is None:
        repo_names = distro.repositories.keys()
    for repo in repo_names:
        yield from distro.repositories[repo].release_repository.package_names
